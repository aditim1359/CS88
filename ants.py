...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.is_container == True and insect.is_container == False:
                    if self.ant.contained_ant == None:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect.imposter is not True:
                return
            # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        ant = self.place.ant
        if ant is not None:
            return ant.blocks_path is True
        else:
            return False
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        entrance_pos = 0
        while place != beehive:
            if entrance_pos>= self.min_range and entrance_pos<= self.max_range:
                if place.bees != []:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter == True:
            ...
            while place.exit:
                ant = place.exit.ant
                if ant is not None and ant not in self.ants_doubled:
                    ant.damage = 2* ant.damage
                    self.ants_doubled.append(ant)
                #bodyguard or tank ant, double damage of inside ant too
                if isinstance(ant, BodyguardAnt) and ant.contained_ant is not None and ant.contained_ant not in self.ants_doubled:
                    ant.contained_ant.damage = 2* ant.contained_ant.damage
                    self.ants_doubled.append(ant.contained_ant)
                place = place.exit
